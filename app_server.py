...
def match_stem_in_text(stem, text, tokens):
    ...
    if ' ' in s:
        return s in text
    if len(s) <= 3:
        return any(t == s for t in tokens)
    return any(t.startswith(s) or (len(t) >= 6 and len(s) >= 6 and s.startswith(t)) for t in tokens)
...
def fallback_categorize(desc, tx_type):
    ...
    tokens = [t for t in re.split(r'[\s,.;:!?+*\/\\-_()]+', d) if len(t) >= 2]

    if tx_type == 'income':
        for cat, stems in INCOME_STEM_MAP.items():
            if any(match_stem_in_text(s, d, tokens) for s in stems):
                return cat
        return 'Інші доходи'
    else:
        scores = {}
        for cat, stems in EXPENSE_STEM_MAP.items():
            score = sum(1 for s in stems if match_stem_in_text(s, d, tokens))
            if score > 0:
                scores[cat] = score
...
- 'Транспорт та Авто': Таксі (Uklon, Bolt, Uber), громадський транспорт, метро, проїзд, квитки, бензин, паливо, заправки, автомийка, ремонт авто.
- 'Комунальні та Житло': Оренда, комуналка, світло, газ, вода, опалення, інтернет, ремонт житла, товари для дому.
- 'Здоров'я та Спорт': Аптека, ліки, лікарі, аналізи, стоматолог, спортзал, тренування, басейн (НЕ документи і НЕ паспорт!).
...
- 'Інші витрати': Офіційні документи (паспорт, закордонний паспорт, ID-картка, свідоцтво, довідки), держмито, послуги ЦНАП, нотаріус, юридичні послуги, податки, банківські комісії, або витрати, що не підпадають під інші категорії.
...
Тип операції: {tx_type}"""
        ...
        try:
            import requests
            prompt = f"""Ти інтелектуальний фінансовий аналітик додатка MonoFinance.
Твоє завдання — класифікувати транзакцію з мінімальним порогом фільтрації. Будь-яка їжа та випічка належить до 'Продукти харчування' або 'Кафе та ресторани'. Документи та паспорт належать до 'Інші витрати'.
